...
/// This matches the Linux kernel implementation used by ext4.
...
/// Raw CRC-32C (no final XOR) — for chaining like Linux kernel
...
/// Standard CRC-32C with final XOR (init=0xFFFFFFFF, XOR=0xFFFFFFFF)
pub fn crc32c(data: &[u8]) -> u32 {
    crc32c_raw(data, 0xFFFFFFFF) ^ 0xFFFFFFFF
}

/// Compute ext4 checksum seed from superblock UUID
...
pub fn inode_checksum(
    ...
    // Chain: seed → inode_num (LE) → generation (LE)
    ...
    // CRC covers: inode_raw[0..124] + 0x0000 (checksum_lo placeholder) + inode_raw[126..128]
    let crc = crc32c_raw(&inode_raw[..124], crc);
    let crc = crc32c_raw(&[0, 0], crc); // checksum_lo = 0
    let crc = crc32c_raw(&inode_raw[126..128.min(inode_raw.len())], crc);

    // For large inodes (>128): extra fields after offset 128
    if inode_size > 128 && inode_raw.len() > 128 {
        ...
        if extra_end > extra_start + 4 {
            // inode_raw[128..130] = extra fields, [130..132] = checksum_hi
            let crc = crc32c_raw(&inode_raw[extra_start..130], crc);
            let crc = crc32c_raw(&[0, 0], crc); // checksum_hi = 0
            if extra_end > 132 {
...
pub fn dir_block_checksum(
    ...
    let crc = crc32c_raw(&ino_bytes, seed);
    let crc = crc32c_raw(&gen_bytes, crc);
    // CRC covers block_data[0..block_size-12] (last 12 bytes are checksum tail)
...
pub fn group_desc_checksum(seed: u32, group_num: u32, gd_raw: &[u8]) -> u16 {
    ...
    // GD[0..30] + 0x0000 + GD[32..]
    let crc = crc32c_raw(&gd_raw[..30.min(gd_raw.len())], crc);
    let crc = crc32c_raw(&[0, 0], crc); // checksum placeholder
