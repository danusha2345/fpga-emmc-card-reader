...
impl<'a> Ext4Fs<'a> {
    pub fn open(io: &'a mut dyn SectorIO, partition_start_lba: u64) -> Result<Self> {
        let sb_sector = partition_start_lba as u32 + 2;
        let sb_data = io.read_sectors(sb_sector, 2)?;
        ...
        let gd_data = io.read_sectors(gd_sector, gd_sectors as u16)?;
        ...
        for i in 0..sb.num_groups as usize {
            ...
            if offset + sb.desc_size as usize > gd_data.len() { break; }
            ...
            if sb.metadata_csum && gd_raw.len() >= 32 {
                ...
                if stored != computed {
                    eprintln!("Warning: group descriptor {} checksum mismatch", i);
                }
            }
        ...
        Ok(Self { io, partition_start_lba, sb, group_descs })
    }

    pub fn read_block(&mut self, block_num: u64) -> Result<Vec<u8>> {
        let sector = self.partition_start_lba as u32 + (block_num * self.sb.block_size as u64 / 512) as u32;
        let sectors_per_block = (self.sb.block_size / 512) as u16;
        self.io.read_sectors(sector, sectors_per_block)
    }

    pub fn write_block(&mut self, block_num: u64, data: &[u8]) -> Result<()> {
        let sector = self.partition_start_lba as u32 + (block_num * self.sb.block_size as u64 / 512) as u32;
        self.io.write_sectors(sector, data)
    }
